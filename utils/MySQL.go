...
func Init_gorm() *gorm.DB {
	//配置MySQL连接参数
	username := configs.GetStringConfig("mysql.username") //账号
	password := configs.GetStringConfig("mysql.password") //密码
	host := configs.GetStringConfig("mysql.host")         //数据库地址，可以是Ip或者域名
	port := configs.GetIntConfig("mysql.port")            //数据库端口
	Dbname := configs.GetStringConfig("mysql.dbname")     //数据库名
